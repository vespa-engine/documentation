...
<table className="vespa-table">
  ...
  <tbody>
    <tr>
      ...
      <td>A handler that modifies a request before dispatching it to the <code>{`ProcessingHandler`}</code>. This handler is also used in the <a href="/en/learn/tutorials/http-api" aria-label="HTTP API tutorial">HTTP API tutorial</a>. Note that since this depends on ProcessingHandler you must add <code>{`processing`}</code> to your <code>{`container`}</code> tag to use it. If you want to issue Queries instead, have com.yahoo.search.searchchain.ExecutionFactory injected instead and use it to create executions and call search/fill on them.</td>
    </tr>
  </tbody>
</table>

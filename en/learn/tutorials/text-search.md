...
The sample data must be converted to Vespa [JSON feed format](../../reference/schemas/document-json-format.html).
...
The [string](../../reference/schemas/schemas.html#string) data type is used to represent both unstructured and structured texts, 
...
- `<documents>` assigns the document types in the _schema_  to content clusters —
  ...
  (See also the [reference](../../reference/applications/services/content.html) for more on content cluster setup.)
...
change the [grammar](../../reference/querying/yql.html#grammar) to specify how the user query text is parsed into a query execution plan.

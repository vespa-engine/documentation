...
accuracy. We will create a small script that converts the above output to [Vespa JSON document](../../reference/schemas/document-json-format.html) format. Create a `convert.py` file:
...
and how Vespa should index and [match](../../reference/schemas/schemas.html#match) them.
...
The [string](../../reference/schemas/schemas.html#string) data type represents both unstructured and structured texts, 
...
[Fieldset](../../reference/schemas/schemas.html#fieldset) allows searching across multiple fields. Defining `fieldset` does not 
add indexing/storage overhead. String fields grouped using fieldsets must share the same 
[match](../../reference/schemas/schemas.html#match) and [linguistic processing](../../linguistics/linguistics.html) settings because
...
trained with cosine similarity, which maps to Vespa's `angular` [distance-metric](../../reference/schemas/schemas.html#distance-metric) for 
...
What is going on here is that we are combining the two top-k query operators using a boolean OR (disjunction). 
...
We add a few more rank profiles to the schema that combine the two retrieval strategies in different ways.

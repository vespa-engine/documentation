...
| **provides** | optional |  |  | A space-separated list of names that represents what this searcher produces. For more information on provides, before and after, see [chained components](/en/applications/chaining) |
...
| Attribute | Required | Value | Default | Description |
| --- | --- | --- | --- | --- |
| **max** | optional | number | equal to `<threads>` | The maximum number of threads relative to vCPU cores. Value must be greater than or equal to `<threads>`. |

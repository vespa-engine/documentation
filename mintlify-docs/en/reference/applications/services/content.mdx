...
Contained in [proton](#proton). The number of data copies that are indexed (*ready*) and hence searchable.

Vespa Cloud: Searchable copies *per group*, capped at [redundancy](#redundancy). Default 1 with multiple groups, otherwise 2 (or redundancy, if lower).
...
In clusters using only [streaming search](/en/performance/streaming-search) there is no index, so a searchable copy costs no extra resources - the default is redundancy, making failover instant.

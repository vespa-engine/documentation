...
Individual output aggregators can also be assigned a label by adding `as(mylabel)` to the aggregator inside `output(...)`. This labels the aggregation result, not the group list.

...
Sometimes all that is needed is letting the standard query framework reply for more paths than standard. This is possible by adding extra [binding](/en/reference/applications/services/search#binding)s inside the `<search>` element in `services.xml`. Writing a custom [request handler](/en/applications/request-handlers) is recommended if the application is a standalone HTTP API, and especially if there are properties used with the same name as those in the [Query API](/en/reference/api/query). A request handler may query the search components running in the same container without any appreciable overhead:

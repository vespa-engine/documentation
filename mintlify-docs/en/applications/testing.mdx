...
`feed-service` is the endpoint of the container cluster with `<document-api>` in [services.xml](/en/reference/applications/services/services). `query-service` is the endpoint of the container cluster with `<search>` in [services.xml](/en/reference/applications/services/services).

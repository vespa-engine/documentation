...
- Numeric fields: an all-encompassing [range](/en/reference/querying/yql#numeric) - `where range(size, -Infinity, Infinity)` matches documents where the field is set.
- String _attributes_: a [regular expression](/en/querying/text-matching#regular-expression-match) - `where album matches "^."` matches documents with a non-empty value. Add [fast-search](/en/reference/schemas/schemas#attribute) to the attribute to make this efficient on a large corpus.
</Accordion>

...
An application package has one or more schemas. There is no hard limit to number of schemas, the largest application have hundreds.

A schema defines how data is stored in a [content cluster](/en/learn/overview#content-clusters). The most common approach is storing all schemas in a single content cluster. Use more content clusters to isolate load or scale the clusters differently.

A schema defines a [document type](/en/schemas/documents). One can store documents with the same document type in different content clusters. Use cases are A/B testing and hot/cold variations of data. Vespa will update documents in all content clusters with the same document type, the mapping is set in [services.xml](/en/reference/applications/services/content#document) using *type*.

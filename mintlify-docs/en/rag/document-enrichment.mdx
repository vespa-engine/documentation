...
Document enrichment enables automatic generation of document field values using large language models (LLMs) or custom code during feeding. It can be used to transform raw text into a more structured representation or expand it with additional contextual information.
...
Enrichments are defined in a schema using a [generate indexing expression](/en/reference/writing/indexing-language#generate).
For example the following schema defines two [synthetic fields](/en/operations/reindexing) with `generate`:
...
Configuration details for local LLM and OpenAI client are covered in [local LLM](/en/rag/local-llms)
and [OpenAI client](/en/rag/external-llms) documentation.
...
[Local LLMs documentation](/en/rag/local-llms) explains how to configure 
...
parameters so that `contextSize / parallelRequests >= maxPromptTokens + maxTokens`. Also consider that larger `contextSize` takes longer to process.
...
In practice, GPU is highly recommended for running local LLMs. It provided order of magnitude speedup compared to CPU.
...
[document JSON format](/en/reference/schemas/document-json-format) used for feeding.
...
As usual with Vespa, existing functionality can be extended by developing [custom application components](/en/applications/developer-guide).

...
Now documents can include promotions as follows ([document JSON syntax](/en/reference/schemas/document-json-format)):
...
To see why this retrieves the right price, notice that `(attribute(startTime) < now)`[is a shorthand for](../reference/ranking/ranking-expressions#non-primitive-functions)
...
Example [JSON](/en/reference/schemas/document-json-format#tensor) document with the vector [1.0, 2.0, 3.0, 5.0], using indexed tensors short form:
...
Note that this example calculates the dot product for every document retrieved by the query. Consider using [approximate nearest neighbor](/en/querying/approximate-nn-hnsw) search with `distance-metric` [dotproduct](/en/reference/schemas/schemas#distance-metric).
...
This weight tensor can be added as a [constant tensor](/en/reference/schemas/schemas#constant) to the application package, say `constant(model)`. With that we can compute the model in a rank profile by the expression
...
that is set as `query(query_vector)` in a searcher as specified in [query feature](/en/ranking/ranking-expressions-features#using-query-variables).
...
Tensors with mapped dimensions look similar to maps, but are more general. What if all needed is a simple map lookup? See [tensor performance](/en/performance/feature-tuning#mapped-lookups) for more details.

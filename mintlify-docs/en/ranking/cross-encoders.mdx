...
sidebarTitle: "Cross-encoder transformer ranking"
---

[Cross-Encoder Transformer](https://blog.vespa.ai/pretrained-transformer-language-models-for-search-part-4/) based text ranking models are generally more effective than [text embedding](/en/rag/embedding) models as they take both the query and the document as input with full cross-attention between all the query and document tokens. 
...
For exporting models from HF to [ONNX](/en/ranking/onnx), we recommend the [Optimum](https://huggingface.co/docs/optimum/main/en/index) library. Example usage for two relevant ranking models.
...
These two example ranking models use different language model [tokenization](/en/reference/rag/embedding#huggingface-tokenizer-embedder) and also different transformer inputs.
...
Add the generated `model.onnx` and `tokenizer.json` files from the `ranker` directory created by Optimum to the Vespa [application package](/en/basics/applications):
...
To speed up inference, Vespa avoids re-tokenizing the document tokens, so we need to configure the [huggingface-tokenizer-embedder](/en/reference/rag/embedding#huggingface-tokenizer-embedder) in the `services.xml` file:
...
The above will concat the title and body input document fields, and input to the `hugging-face-tokenizer` tokenizer which saves the output tokens as float (101.0). To use the generated `tokens` tensor in ranking, the tensor field must be defined with [attribute](/en/content/attributes).

## Using the cross-encoder model in ranking 

Cross-encoder models are not practical for *retrieval* over large document volumes due to their complexity, so we configure them using [phased ranking](/en/ranking/phased-ranking). 
...
At query time, we need to tokenize the user query using the [embed](/en/rag/embedding#embedding-a-query-text) support. 
...
The retriever (query + first-phase ranking) can be anything, including [nearest neighbor search](/en/querying/nearest-neighbor-search) a.k.a. dense retrieval using bi-encoders.
...
- The number of hits that are re-ranked [rerank-count](/en/reference/schemas/schemas#globalphase-rerank-count) Complexity is linear with the number of hits that are re-ranked.
...
When using [multi-vector indexing](https://blog.vespa.ai/semantic-search-with-multi-vector-indexing/) we can do the following to feed the best (closest) paragraph using the [closest()](../reference/ranking/rank-features#closest(name)) feature into re-ranking with the cross-encoder model. 

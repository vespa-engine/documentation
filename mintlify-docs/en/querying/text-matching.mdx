...
A regular expression can also be used to select documents where a string attribute has a *non-empty* value - `^.` matches any value with at least one character:

```bash
$ vespa query 'select * from music where album matches "^."'
```

This works on any string attribute field. Without *fast-search*, the expression is evaluated over the value of every document, which is slow on a large corpus. Add [fast-search](../reference/schemas/schemas#attribute) to the attribute to make this more efficient. For the complementary query - listing documents *missing* a value - see [count and list fields with NaN](/en/querying/grouping#count-fields-with-nan).

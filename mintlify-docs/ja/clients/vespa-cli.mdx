...
Vespa CLI は Vespa のコマンドライン・クライアントです。ランタイム依存関係のない単一バイナリで、Linux、macOS、Windows で利用できます。Vespa CLI では次のことができます。

- [サンプル・アプリケーション](https://github.com/vespa-engine/sample-apps/)リポジトリのクローン
- ローカルまたはリモートで稼働している Vespa 環境へのアプリケーションのデプロイ
- [Vespa Cloud](/) へのアプリケーションのデプロイ
- ドキュメントのフィードと[クエリー](/ja/querying/query-language#query-examples)
- 自動認証付きのカスタムリクエストの送信
- [vespa auth api-key](/ja/reference/clients/vespa-cli/vespa_auth_api-key) によるデプロイ操作の自動化

Vespa CLI のインストール:

- Homebrew: `brew install vespa-cli`
- Mise: `mise use vespa-cli`
- [GitHub からダウンロード](https://github.com/vespa-engine/vespa/releases)

Vespa CLI の基本的な使い方を学ぶには、[クイックスタート・ガイド](/ja/basics/deploy-an-application-local)または[以下のチートシート](#cheat-sheet)を参照してください。

個々の Vespa CLI コマンドとそのオプションのドキュメントについては、[リファレンス・ドキュメント](/ja/reference/clients/vespa-cli/vespa)を参照してください。このドキュメントは CLI にも同梱されており、`vespa help
  <command>` または `man vespa-<command>` からアクセスできます。
...
`.vespa` ディレクトリはホームディレクトリまたは cwd に置く必要があります。Vespa Cloud のエンドポイントを使用する場合は、`vespa config set target cloud` を実行することを忘れないでください。
...
- `vespa feed` の入力ファイルには、フィード操作の JSON 配列、または 1 行につき 1 つの JSON 操作（[JSONL](https://jsonlines.org/)）のいずれかを含めます。
- ドキュメントをフィードまたはアクセスする前に、コンテナーで [\<document-api\>](/ja/reference/applications/services/container#document-api) を有効にする必要があります - [例](https://github.com/vespa-engine/sample-apps/blob/master/album-recommendation/app/services.xml)を参照してください。
- 自動化については、[GitHub Action](https://github.com/vespa-engine/documentation/blob/master/.github/workflows/feed.yml) での使用例を参照してください。このアクションは、GitHub でのセキュリティ管理を容易にするために、`VESPA_CLI_DATA_PLANE_CERT` と `VESPA_CLI_DATA_PLANE_KEY` のセキュリティ資格情報を使用します。
</Note> 

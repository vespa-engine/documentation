...
- Describe the dataset used as well as any information connected to the data
  that we consider relevant to this tutorial.
- Show how to setup a basic blog post search engine application with Vespa.
...
Kaggle](https://www.kaggle.com/c/predict-wordpress-likes/data) and were
...
<tr><th>categories</th><td>array of strings representing the categories the blog post were assigned to</td></tr>
</tbody>
</table>
...
<tr><th>dt</th><td>date of the interaction, format <em>yyyy-mm-dd hh:mm:dd</em></td></tr>
</tbody>
</table>
...
Example: For a simple blog post search application we could create the document
type `blog_post`, which could be defined in a search definition with the
following fields:
...
<dl class="dl-horizontal">
  ...
  <dt>date_gmt</dt> <dd>of type long (to store the date)</dd>
</dl>

The data fed into Vespa must match the structure of the search definition,
and the hits returned when searching will be of the same format as well.
...
The _search definition_ is a required part of an application package -
...
_container_ defines the [container](../jdisc/index.html) cluster for document, query and result processing.
The _document-api_ element sets up the _document_ endpoint for feeding.
The _search_ element sets up the _search_ endpoint for Vespa queries.
The default port for both is 8080.
_nodes_ defines the number of nodes required per service. Refer to the [reference](../reference/services-container.html).

_content_ defines how documents are stored and searched within Vespa.
_redundancy_ configures how many copies to keep of each document.
_nodes_ defines hosts for the content cluster.
_documents_ assigns the document types in the _search definition_ -
setting the _mode_ attribute to _index_ for the document type
enables indexed search (as opposed to
[streaming search](../streaming-search.html)).
Increase the content cluster capacity by adding node elements - see
[elastic Vespa](../elastic-vespa.html). Refer to the
[reference](../reference/services-content.html).
...
_document_ is wrapped inside another element called _search_.
The name following these elements (<em>blog_post</em>) must be exactly the same for both.

The field property _indexing_ configures the
so-called _indexing pipeline_ for a field, which defines how
Vespa will treat input during indexing - see
[indexing language](../reference/advanced-indexing-language.html).
Each part of the indexing pipeline is separated by a pipe character (|), and the two keywords
used above - _index_ and _summary_ - are the most common ones:
...
- *summary:* means that this field will be part of the
...
Searching with Vespa is done using a simple HTTP interface, basic GET requests.
The general form of an unstructured search request parsing end user input is:
...
The query string contains the specification of which results the search should
return. This specification is written in the Vespa Query Language. The Vespa
Query Language comes in two flavors:

- The *simple* query language is intended to be usable directly by end users.
  This is the default in Vespa.
- The *advanced* query syntax is intended for programmatic use and are formulated
  in YQL, which should be used when building applications.
...
Given the configuration - where the fields title and content are part of the
default field set defined in the search definition - any document containing
...
- The field named documentid in the output and how it matches the value
...
- Each hit has a property named relevance, which indicates how well the given
  document matched our query using a pre-defined default ranking function. You
...
Above, ranking and relevance was briefly mentioned.
But what is really the relevance of a hit?
And how can one change the way relevance calculations are done?
It is time to introduce _rank profiles_ and _rank expressions_ –
simple, yet extremely powerful methods for tuning the relevance.
...
but the essence is that it is all a measure of how well the document matches the terms in the query.
...
Use a number of built-in functions and expressions to create highly specialized rank expressions.


### Blog popularity signal

Include the notion of blog popularity into the ranking function.
...
Include the `post_popularity` rank-profile in _src/main/application/searchdefinitions/blog_post.sd_:

        rank-profile post_popularity inherits default {
            first-phase {
                expression:nativeRank(title, content) + if(isNan(attribute(popularity)) == 1, 0,attribute(popularity))
            }
        }

just below the `post` rank-profile near the bottom of the search definition.
Also add a `popularity` field:
...
- `rank-profile post_popularity inherits default`
  ...
  and that it should inherit all its properties from the default rank-profile.
  This means that any properties we do not explicitly
  specify are used as-is from the default rank profile instead.
...
- `expression:nativeRank(title, content) + if(isNan(attribute(popularity)) == 1, 0,attribute(popularity))`
  ...
  This ranks documents. Vespa is configured to calculate the relevance for each document
  by first finding the `nativeRank(title, content)` for the document,
  and then add the value of the static document value `attribute(popularity)` if it's set,
  for those documents that do not have a popularity value, the score is set to 0.
  The sum of these two factors is the final relevance for a given document.
...
which means that Vespa will keep the contents of the field in memory at all time.
You can still use this field in searches and treat it almost as any other field,
but conceptually it is quite different than regular Vespa indexes –
where new documents are only kept in memory for a period of time,
and then gradually moved to a disk-based index.
The downside of attributes is that they make Vespa use more memory per document
as the actual field contents is stored in memory.
Searching attribute fields is by default done without any index support unless
_attribute:fast-search_ is set.

### Defining an attribute field

A field with indexing attribute will be present in memory at all time for very
fast access, an example is found in _blog_post.sd_:
...
query for "music AND festival"? We got seven document hits, and – only
considering date and relevancy for each document – the output and order looked
like this:
